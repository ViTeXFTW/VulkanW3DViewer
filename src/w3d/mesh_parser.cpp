  ...
  while (reader.position() < endPos) {
    ...
    switch (header.type) {
      ...
      case ChunkType::TEXCOORDS: {
        ...
        for (size_t i = 0; i < count; ++i) {
          mesh.texCoords.push_back(reader.readVector2());
        }
        break;
      }
      ...
      case ChunkType::VERTEX_MATERIALS: {
        ...
        while (!subReader.atEnd()) {
          ...
          if (subHeader.type == ChunkType::VERTEX_MATERIAL) {
            mesh.vertexMaterials.push_back(
                parseVertexMaterial(subReader, subHeader.dataSize()));
          } else {
        ...
        while (!subReader.atEnd()) {
          ...
          if (subHeader.type == ChunkType::TEXTURE) {
            mesh.textures.push_back(
                parseTexture(subReader, subHeader.dataSize()));
          } else {
            subReader.skip(subHeader.dataSize());
          }
        }
...
VertexMaterial MeshParser::parseVertexMaterial(ChunkReader& reader,
                                               uint32_t chunkSize) {
  VertexMaterial mat;
  size_t endPos = reader.position() + chunkSize;
...
TextureStage MeshParser::parseTextureStage(ChunkReader& reader,
                                           uint32_t chunkSize) {
  TextureStage stage;
  size_t endPos = reader.position() + chunkSize;
  ...
  while (reader.position() < endPos) {
    ...
    switch (header.type) {
      ...
      case ChunkType::STAGE_TEXCOORDS: {
        ...
        for (size_t i = 0; i < count; ++i) {
          stage.texCoords.push_back(reader.readVector2());
        }
        break;
      }
...
MaterialPass MeshParser::parseMaterialPass(ChunkReader& reader,
                                           uint32_t chunkSize) {
  MaterialPass pass;
  size_t endPos = reader.position() + chunkSize;
