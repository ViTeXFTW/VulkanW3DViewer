  ...
  while (reader.position() < endPos) {
    ...
    case ChunkType::HLOD_AGGREGATE_ARRAY: {
      ...
      while (!subReader.atEnd()) {
        ...
        if (subHeader.type == ChunkType::HLOD_SUB_OBJECT) {
          hlod.aggregates.push_back(parseSubObject(subReader, subHeader.dataSize()));
        } else {
    ...
    case ChunkType::HLOD_PROXY_ARRAY: {
      ...
      while (!subReader.atEnd()) {
        auto subHeader = subReader.readChunkHeader();
        if (subHeader.type == ChunkType::HLOD_SUB_OBJECT) {
          hlod.proxies.push_back(parseSubObject(subReader, subHeader.dataSize()));
        } else {
          subReader.skip(subHeader.dataSize());
        }
      }
...
HLodSubObject HLodParser::parseSubObject(ChunkReader &reader, uint32_t /*dataSize*/) {
